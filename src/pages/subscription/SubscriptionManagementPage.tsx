  ...
  return (
    <div className="space-y-8 opacity-0 animate-fade-up [animation-fill-mode:forwards] text-zinc-900">
      ...
      <header className="border-b border-zinc-300 pb-6">
        <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-600">Billing & plan</p>
        <h1 className="mt-2 text-2xl font-bold tracking-tight text-zinc-900 sm:text-3xl">Subscription Management</h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:gap-8 xl:grid-cols-12">
        <section className="relative overflow-hidden rounded-2xl border border-zinc-300 bg-white shadow-glass xl:col-span-8">
          ...
          <div className="flex flex-col gap-4 border-b border-zinc-300 bg-zinc-100 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-6">
            <div className="flex min-w-0 items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1.5 text-sm text-zinc-600">Renewals, payments, and subscription lifecycle.</p>
              </div>
            </div>
            <div className="shrink-0 rounded-xl border border-zinc-300 bg-white px-4 py-3 text-left sm:text-right">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-zinc-500">Next billing</p>
          ...
          <div className="p-5 sm:p-6">
            {loading ? (
              ...
              <div className="overflow-hidden rounded-xl border border-zinc-300 bg-white">
                <div className="grid md:grid-cols-2 md:divide-x md:divide-zinc-300">
                  <div className="border-b border-zinc-300 p-5 sm:p-6 md:border-b-0">
                    ...
                    {contact ? (
                      <dl className="mt-4 space-y-3 text-sm">
                        ...
                        <div>
                          ...
                          <dd className="mt-0.5 break-all font-medium text-zinc-900">{contact.email}</dd>
                        </div>
                      </dl>
                    ) : (
                  ...
                  <div className="p-5 sm:p-6">
                    <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-500">Plan actions</p>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Link
                        to="/subscription"
                        className="inline-flex items-center justify-center rounded-lg border border-zinc-300 bg-zinc-100 px-4 py-2 text-xs font-semibold text-zinc-800 transition hover:border-zinc-400 hover:bg-zinc-200"
                      >
                        Change plan
                      </Link>
                      <button
                        ...
                        disabled={busy}
                        className="inline-flex items-center justify-center rounded-lg bg-zinc-700 px-4 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-zinc-600 disabled:opacity-50"
                      >
        ...
        <section className="flex flex-col overflow-hidden rounded-2xl border border-zinc-300 bg-white shadow-glass xl:col-span-4">
          <div className="border-b border-zinc-300 bg-zinc-100 px-5 py-4">
            <h3 className="text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-500">Billing summary</h3>
            <p className="mt-1 text-xs text-zinc-600">Snapshot of how you pay and what is open.</p>
          </div>
          <div className="flex flex-1 flex-col divide-y divide-zinc-300">
            <div className="px-5 py-4">
              <p className="text-xs text-zinc-500">Default payment method</p>
              {loading ? (
                <Skeleton className="mt-2 h-5 w-40" />
              ) : defaultMethod ? (
                <p className="mt-2 font-mono text-sm font-medium text-zinc-900">
                  {defaultMethod.brand.toUpperCase()} •••• {defaultMethod.last4}
                </p>
              ) : (
                <p className="mt-2 text-sm text-zinc-600">No payment method on file</p>
              )}
            </div>
            <div className="flex items-baseline justify-between gap-3 px-5 py-4">
              ...
              {loading ? (
                ...
                <p className="text-2xl font-bold tabular-nums text-zinc-900">{invoices.length}</p>
              )}
            </div>
            <div className="flex items-baseline justify-between gap-3 px-5 py-4">
              <p className="text-xs text-zinc-500">Open support requests</p>
              {loading ? (
                <Skeleton className="h-8 w-10" />
              ) : (
                <p className="text-2xl font-bold tabular-nums text-zinc-900">{pendingCount}</p>
              )}
            </div>
          </div>
        </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-zinc-300 bg-white shadow-glass">
        <div className="border-b border-zinc-300 bg-zinc-100 px-5 py-4 sm:px-6">
          <h2 className="text-[10px] font-semibold uppercase tracking-[0.16em] text-zinc-500">Invoices</h2>
          <p className="mt-1 text-xs text-zinc-600">Receipts and PDFs from completed charges.</p>
        </div>
